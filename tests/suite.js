...
(async () => {
  ...
  eq(Object.keys(byCode).map(Number).sort((a, b) => a - b), [...Array(382).keys()], "codes are 0..381 with no gaps or reuse");
  eq(["furry","xenophilia-tentacles","trampling-barefoot","trampling-shoes","rubber-band-snapping","forced-drinking-beer-cider","irrumatio-to-vomiting","bukkake","cum-in-eyes","nerd-hikikomori","humiliating-body-writing"].map(id => ids.indexOf(id) >= 0), Array(11).fill(true), "11 added items present");
  ...
  const enNames = ids.map(id => KC.i18n.item(id, "en").name); eq(new Set(enNames).size, 382, "EN names unique");
  ...
  eq(d.querySelectorAll(".item").length, 382, "382 rows rendered");
  ...
  eq(d.getElementById("progress").textContent, "Отмечено 4 из 382 практик", "progress text");
  ...
  eq(d.getElementById("progress").textContent, "4 of 382 practices marked", "EN progress");
  ...
  eq(d.getElementById("progress").textContent, "4 de 382 práticas marcadas", "PT progress");
  ...
  eq(Object.keys(KCn.codec.decode(KCn.codec.encode({ items: allNew, meta: {} })).items).length, 382, "fully filled 382-item link round-trips");
  ...
  eq(p.d.getElementById("progress").textContent, "382項目中 4項目にチェック済み", "JA progress");
