...
(async () => {
  ...
  eq([...vsel.options].map(o => o.textContent), ["Все пункты", "Только без ответа", "Только новые", "Отвеченные, по ответам", "Да / Обожаю / Может"], "Show menu labels");
  ...
  ok(!p.d.getElementById("filtDot").hidden, "Filters button shows a dot while a filter in it is on");
  ...
  p.d.getElementById("tplName").value = "Вечер";
  click(p.w, p.d.getElementById("tplShareBtn"));
  const tplLink = p.d.getElementById("shareLink").value, td = p.KC.codec.decode(tplLink);
  ok(/&ti=[a-z0-9]{6}&tn=/.test(tplLink) && !/_/.test(tplLink), "template link carries ti= and tn=, no “_”");
  eq([td.tpl.name, Object.keys(td.items).sort(), td.damaged, td.uid], ["Вечер", Object.keys(A5).sort(), false, "ANNA01"], "template link = answered items + the sender's answers");
  ok(/Ссылка-шаблон «Вечер», пунктов: 5/.test(p.d.getElementById("shareKind").textContent) && !p.d.getElementById("shareBack").hidden, "share window says which link is shown");
  eq(tplLink.length - plain.length, "&ti=xxxxxx&tn=".length + encodeURIComponent("Вечер").length, "template adds only its id and name to the link");
  ...
  let tl = JSON.parse(p.w.localStorage.getItem("checklist-templates-v1"));
  eq([tl.length, tl[0].own, tl[0].name, tl[0].ids.length, tl[0].tid], [1, true, "Вечер", 5, td.tpl.id], "“Save as my template”: in My templates, same id as the shared one");
  click(p.w, p.d.getElementById("tplSaveBtn"));
  eq(JSON.parse(p.w.localStorage.getItem("checklist-templates-v1")).length, 1, "saving the same name again updates it");
  click(p.w, p.d.getElementById("mineBtn"));
  ok(/Вечер/.test(p.d.getElementById("mineTplList").textContent) && /пунктов: 5/.test(p.d.getElementById("mineTplList").textContent), "My lists → My templates");
  const senderStorage = p.storage();
  // empty list cannot make a template
  ...
  const recOwn = { local: { "checklist-lang": "ru", "practices-checklist-v1": JSON.stringify({ name: "Boris", uid: "BORIS1", meta: { role: "dom" }, items: { hugging: { interest: "yes" }, "sleep-sacks": { interest: "love" } } }) }, session: {} };
  let r3 = open("form", { hash: tplLink.split("#")[1], storage: recOwn });
  ok(/по шаблону «Вечер»/.test(r3.d.getElementById("bannerText").textContent) && /сохранён в «Полученные → Шаблоны»/.test(r3.d.getElementById("bannerText").textContent), "banner: list by template, template saved");
  eq(visP(r3).sort(), Object.keys(A5).sort(), "only the template's items are shown");
  eq(r3.d.getElementById("progress").textContent, "Отмечено 5 из 5 практик", "progress counts the template's items");
  tl = JSON.parse(r3.w.localStorage.getItem("checklist-templates-v1"));
  eq([tl.length, tl[0].own, tl[0].name, tl[0].ids.length], [1, false, "Вечер", 5], "template in Received → Templates");
  const rl3 = JSON.parse(r3.w.localStorage.getItem("checklist-saved-profiles-v1"));
  eq([rl3.length, r3.KC.codec.decode(rl3[0].code).tpl.name], [1, "Вечер"], "sender's list in Received, marked as by template");
  click(r3.w, r3.d.getElementById("savedBtn"));
  ok(/по шаблону «Вечер»/.test(r3.d.getElementById("savedList").textContent) && /Вечер/.test(r3.d.getElementById("savedTplList").textContent), "Received shows both");
  eq(JSON.parse(r3.w.localStorage.getItem("practices-checklist-v1")).template, undefined, "opening a template does not change the own list");
  ok(!r3.d.getElementById("bannerTplFill").hidden, "“Fill in by this template” button shown");
  r3 = open("form", { hash: tplLink.split("#")[1], storage: r3.storage() });
  ok(/уже есть в «Полученных»/.test(r3.d.getElementById("bannerText").textContent) && JSON.parse(r3.w.localStorage.getItem("checklist-templates-v1")).length === 1, "same template again: not duplicated");
  click(r3.w, r3.d.getElementById("bannerTplFill"));
  let bo = JSON.parse(r3.w.localStorage.getItem("practices-checklist-v1"));
  eq([bo.template.name, bo.template.ids.length, bo.items], ["Вечер", 5, { hugging: { interest: "yes" }, "sleep-sacks": { interest: "love" } }], "fill by template: own list gets it, own answers kept");
  ok(JSON.parse(r3.w.localStorage.getItem("checklist-my-profiles-v1"))[0].data.template.name === "Вечер", "…and its My lists entry");
  let b3 = open("form", { storage: r3.storage() });
  eq([visP(b3).sort(), b3.d.getElementById("progress").textContent], [Object.keys(A5).sort(), "Отмечено 1 из 5 практик"], "own list by template: only its items");
  ok(/по шаблону «Вечер»/.test(b3.d.getElementById("tplNoteText").textContent) && !b3.d.getElementById("tplNote").hidden, "note above the list");
  eq(b3.d.getElementById("tplSel").value, tl[0].tid, "filter panel shows the template");
  eq(Object.keys(b3.KC.codec.decode(b3.KC.form.shareLink()).items), ["hugging"], "share: only answers inside the template");
  ok(b3.KC.form.state.items["sleep-sacks"], "answer outside the template kept");
  eq(Object.keys(b3.KC.codec.decode(b3.KC.store.ownCode()).items), ["hugging"], "compare gets only the template's answers");
  sh = b3.KC.form.buildSheet().textContent;
  ok(/По шаблону «Вечер», пунктов: 5/.test(sh) && !/Спальный мешок/.test(sh), "PDF: template named, nothing outside it");
  click(b3.w, b3.d.getElementById("mineBtn")); ok(/по шаблону «Вечер»/.test(b3.d.getElementById("mineList").textContent), "My lists: marked as by template");
  // template over template: only the answered items
  click(b3.w, b3.d.querySelector('.item[data-id="chains"] .scale button[data-v="yes"]'));
  click(b3.w, b3.d.getElementById("shareBtn"));
  ok(/Включён шаблон «Вечер»/.test(b3.d.getElementById("shareTplNote").textContent), "share window notes the template");
  b3.d.getElementById("tplName").value = "Мини"; click(b3.w, b3.d.getElementById("tplShareBtn"));
  const mini = b3.KC.codec.decode(b3.d.getElementById("shareLink").value);
  eq([mini.tpl.name, Object.keys(mini.items).sort()], ["Мини", ["chains", "hugging"]], "template made inside a template = answered items of it");
  // switching the template off shows everything again
  const tsel = b3.d.getElementById("tplSel"); tsel.value = ""; tsel.dispatchEvent(new b3.w.Event("change"));
  eq([visP(b3).length, b3.KC.form.state.template, b3.d.getElementById("tplNote").hidden], [446, undefined, true], "no template: all items again");
  ok(b3.KC.codec.decode(b3.KC.form.shareLink()).items["sleep-sacks"], "…and all answers in the link");
  tsel.value = tl[0].tid; tsel.dispatchEvent(new b3.w.Event("change"));
  eq(b3.KC.form.state.template.name, "Вечер", "picking the template in the filter panel binds it");
  // sender updates the template: recipient's copy replaced, not duplicated
  p = open("form", { storage: senderStorage });
  click(p.w, p.d.querySelector('.item[data-id="spanking-hand"] .scale button[data-v="yes"]'));
  click(p.w, p.d.getElementById("shareBtn")); p.d.getElementById("tplName").value = "Вечер"; click(p.w, p.d.getElementById("tplShareBtn"));
  const tplLink2 = p.d.getElementById("shareLink").value;
  eq(p.KC.codec.decode(tplLink2).tpl.id, td.tpl.id, "same name from the same list keeps the template id");
  r3 = open("form", { hash: tplLink2.split("#")[1], storage: r3.storage() });
  tl = JSON.parse(r3.w.localStorage.getItem("checklist-templates-v1"));
  eq([tl.length, tl[0].ids.length], [1, 6], "newer version of a template updates the received copy");
  ok(/обновлён до этой версии/.test(r3.d.getElementById("bannerText").textContent), "banner says the template was updated");
  // own template link on the sender's device
  const ps = open("form", { hash: tplLink.split("#")[1], storage: senderStorage });
  ok(/ваш собственный шаблон/.test(ps.d.getElementById("bannerText").textContent) && JSON.parse(ps.w.localStorage.getItem("checklist-templates-v1")).every(x => x.own), "own template link: not added to Received");

  S("Templates: someone's list through a template");
  let rv = open("form", { hash: bobLink, storage: r3.storage() });
  click(rv.w, rv.d.getElementById("bannerTpl"));
  ok(!rv.d.getElementById("filtBar").hidden, "“Through a template…” opens the filter panel");
  const rsel = rv.d.getElementById("tplSel");
  ok([...rsel.querySelectorAll("optgroup")].map(g => g.label).indexOf("Полученные шаблоны") >= 0, "picker lists received templates");
  rsel.value = tl[0].tid; rsel.dispatchEvent(new rv.w.Event("change"));
  eq(visP(rv).length, 6, "someone's list cut to the template");
  sh = rv.KC.form.buildSheet().textContent;
  ok(/По шаблону «Вечер»/.test(sh) && sh.indexOf(rv.KC.i18n.item("spanking-hand").name) >= 0, "PDF of someone's list through a template");
  eq(Object.keys(rv.KC.form.shown().items).sort(), ["hugging", "spanking-hand"], "only answers inside the template");
  eq(JSON.parse(rv.w.localStorage.getItem("practices-checklist-v1")).template.name, "Вечер", "own list keeps its own template (viewing does not change it)");
  click(rv.w, heart(rv, "hugging")); ok(rv.KC.form.favList()[0] === "hugging", "hearts work through a template too");
  const cntBefore = JSON.parse(rv.w.localStorage.getItem("checklist-templates-v1")).length;
  ok(cntBefore === 1 && rv.KC.form.viewTpl.name === "Вечер", "view template not stored in the list");

  S("Templates: library actions, backup");
  let lb = open("form", { storage: rv.storage() });
  click(lb.w, lb.d.getElementById("savedBtn"));
  lb.w.prompt = () => "Мой вечер";
  click(lb.w, lb.d.querySelector('#savedTplList button[data-act="rename"]'));
  tl = JSON.parse(lb.w.localStorage.getItem("checklist-templates-v1"));
  eq([tl[0].label, tl[0].name], ["Мой вечер", "Вечер"], "rename a received template: own label, link name kept");
  r3 = open("form", { hash: tplLink2.split("#")[1], storage: lb.storage() });
  eq(JSON.parse(r3.w.localStorage.getItem("checklist-templates-v1"))[0].label, "Мой вечер", "reopening the link keeps the label");
  const bk2 = r3.KC.store.exportAll();
  ok(bk2.templates.length === 1 && bk2.favs["u:BOB001"] && bk2.mine.some(x => x.data.template), "backup contains templates, favourites and template of a list");
  let q2 = open("form", { storage: { local: {}, session: {} } });
  const res2 = q2.KC.store.importAll(JSON.parse(JSON.stringify(bk2)));
  eq([res2.templates, q2.KC.store.tpl.list().length, q2.KC.store.favs.get("u:BOB001")], [1, 1, ["hugging"]], "restore adds templates and favourites");
  ...
  click(r3.w, r3.d.getElementById("savedBtn"));
  click(r3.w, r3.d.querySelector('#savedTplList button[data-act="del"]'));
  eq(JSON.parse(r3.w.localStorage.getItem("checklist-templates-v1")).length, 0, "delete a received template");
  let b4 = open("form", { storage: r3.storage() });
  eq([b4.KC.form.state.template.name, b4.d.getElementById("tplSel").value], ["Вечер", "cur"], "list keeps its copy after the template is deleted");
  eq(KCn.codec.decode(l1).tpl, undefined, "plain links carry no template");
  // compare picker
  const cp = open("compare", { storage: b4.storage() });
  const copt = [...cp.d.querySelectorAll(".cmp-pick option")].map(o => o.textContent);
  ok(copt.some(x => /по шаблону «Вечер»/.test(x)), "compare picker marks lists by template");
