...
(async () => {
  ...
  eq(Object.keys(byCode).map(Number).sort((a, b) => a - b), [...Array(464).keys()], "codes are 0..463 with no gaps or reuse");
  ...
  const enNames = ids.map(id => KC.i18n.item(id, "en").name); eq(new Set(enNames).size, 464, "EN names unique");
  ...
  eq(d.querySelectorAll(".item").length, 464, "464 rows rendered");
  ...
  eq(d.getElementById("progress").textContent, "Отмечено 4 из 464 практик", "progress text");
  ...
  eq(d.getElementById("progress").textContent, "4 of 464 practices marked", "EN progress");
  ...
  eq(d.getElementById("progress").textContent, "4 de 464 práticas marcadas", "PT progress");
  ...
  ok(bp.d.querySelector(".brand-row #progress") && bp.d.querySelector("#pdfOverlay #onlyMarked") && bp.d.querySelector(".subbar #filtBtn") && bp.d.querySelector("#filtBar #view") && !bp.d.querySelector(".progress-row"), "compact header layout");
  ...
  eq(gp.d.getElementById("progress").textContent, "Отмечено " + gShown + " из 464 практик", "counter = what is shown");
  ...
  eq(Object.keys(KCn.codec.decode(KCn.codec.encode({ items: allNew, meta: {} })).items).length, 464, "fully filled 464-item link round-trips");
  ...
  eq([vis().length, vis().indexOf("hugging"), vis().indexOf("furry")], [462, -1, -1], "unanswered: answered items hidden");
  ...
  sb.value = ""; vsel.value = "all"; vsel.dispatchEvent(new p.w.Event("change")); eq(vis().length, 464, "all items again");
  ...
  eq(tp.d.getElementById("progress").textContent, "เลือกแล้ว 4 จาก 464 รายการ", "TH progress");
  ...
  eq(p.d.getElementById("progress").textContent, "464項目中 4項目にチェック済み", "JA progress");
  ...
  ok(p.d.querySelector(".fav-toggle").classList.contains("on") && p.d.getElementById("filtDot").hidden, "♥ toggle lights up; it is not part of the Filters panel");
  ...
  click(b3.w, tact);
  eq([visP(b3).length, tact.textContent, b3.d.getElementById("tplSel").value], [464, "Только пункты шаблона", ""], "show all: every item, button to go back");
  ...
  eq([visP(b3).length, b3.KC.form.state.template.name], [464, "Evening"], "“No template” in Filters: a view choice only");
  ...
  b3 = open("form", { storage: b3.storage() });
  eq([b3.KC.form.state.name, b3.KC.form.state.template, visP(b3).length], ["Boris", undefined, 464], "switched to my plain list");
  r3 = open("form", { hash: tplLink.split("#")[1], storage: b3.storage() });
  b3 = open("form", { storage: r3.storage() });
  ...
  eq([rv.KC.form.sharedBy, rv.d.getElementById("tplNote").hidden, visP(rv).length], [null, true, 464], "plain list: no template, no note");
  click(rv.w, rv.d.getElementById("bannerTpl"));
  ok(!rv.d.getElementById("filtBar").hidden, "“Show by a template…” opens the filter panel");
  ...
  eq([visP(dz).length, dz.d.getElementById("tplAct").hidden], [464, true], "list by a deleted template: opens with all items");
  ...
  eq([bigD.tpl.ids.length, bigD.damaged], [464, false], "a template of every item round-trips (bitmap form)");
  ...
  ok(hp0.d.querySelector(".brand-row .help-q") && ["start", "share", "lists", "received", "tpl", "filters", "pdf"].every(s => qs.indexOf(s) >= 0), "“?” in the header and in each window: " + qs.join(","));
  ...
  S("Header: Filters panel, ♥ toggle, PDF window");
  let hp = open("form", { storage: own5 });
  click(hp.w, hp.d.getElementById("filtBtn"));
  ok(!hp.d.getElementById("filtBar").hidden && hp.d.getElementById("filtBtn").getAttribute("aria-expanded") === "true", "Filters opens the panel");
  eq([...hp.d.querySelectorAll("#filtBar select")].map(x => x.id), ["tplSel", "view"], "panel: template, then Show");
  ...
  setView(hp, "unanswered"); ok(!hp.d.getElementById("filtDot").hidden, "dot on Filters while Show is not “All items”");
  setView(hp, "all"); ok(hp.d.getElementById("filtDot").hidden, "no dot with All items and no template");
