...
(async () => {
  ...
  eq(Object.keys(byCode).map(Number).sort((a, b) => a - b), [...Array(466).keys()], "codes are 0..465 with no gaps or reuse");
  ...
  const enNames = ids.map(id => KC.i18n.item(id, "en").name); eq(new Set(enNames).size, 466, "EN names unique");
  ...
  eq(d.querySelectorAll(".item").length, 466, "466 rows rendered");
  ...
  eq(d.getElementById("progress").textContent, "Отмечено 4 из 466 практик", "progress text");
  ...
  eq(d.getElementById("progress").textContent, "4 of 466 practices marked", "EN progress");
  ...
  eq(d.getElementById("progress").textContent, "4 de 466 práticas marcadas", "PT progress");
  ...
  eq(gp.d.getElementById("progress").textContent, "Отмечено " + gShown + " из 466 практик", "counter = what is shown");
  ...
  eq(Object.keys(KCn.codec.decode(KCn.codec.encode({ items: allNew, meta: {} })).items).length, 466, "fully filled 466-item link round-trips");
  ...
  eq([vis().length, vis().indexOf("hugging"), vis().indexOf("furry")], [464, -1, -1], "unanswered: answered items hidden");
  ...
  sb.value = ""; vsel.value = "all"; vsel.dispatchEvent(new p.w.Event("change")); eq(vis().length, 466, "all items again");
  ...
  eq(tp.d.getElementById("progress").textContent, "เลือกแล้ว 4 จาก 466 รายการ", "TH progress");
  ...
  eq(p.d.getElementById("progress").textContent, "466項目中 4項目にチェック済み", "JA progress");
  ...
  click(b3.w, tact);
  eq([visP(b3).length, tact.textContent, b3.d.getElementById("tplSel").value], [466, "Только пункты шаблона", ""], "show all: every item, button to go back");
  ...
  eq([visP(b3).length, b3.KC.form.state.template.name], [466, "Evening"], "“No template” in Filters: a view choice only");
  ...
  b3 = open("form", { storage: b3.storage() });
  eq([b3.KC.form.state.name, b3.KC.form.state.template, visP(b3).length], ["Boris", undefined, 466], "switched to my plain list");
  r3 = open("form", { hash: tplLink.split("#")[1], storage: b3.storage() });
  b3 = open("form", { storage: r3.storage() });
  ...
  eq([rv.KC.form.sharedBy, rv.d.getElementById("tplNote").hidden, visP(rv).length], [null, true, 466], "plain list: no template, no note");
  ...
  eq([visP(dz).length, dz.d.getElementById("tplAct").hidden], [466, true], "list by a deleted template: opens with all items");
  ...
  eq([bigD.tpl.ids.length, bigD.damaged], [466, false], "a template of every item round-trips (bitmap form)");
  ...
  eq([visP(hq).length, ts.options[ts.selectedIndex].textContent, ts.classList.contains("on")], [466, "Шаблон…", false], "one pick: no template, the list says “Template…”");
